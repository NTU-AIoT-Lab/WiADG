""" 
	Test target encoder for ADDA 
	Target encoder + Target dataset 
"""
...
def test_tgt(epoch, train_loader, encoder):
	...
	if test_flag == 0:
		# encoder.load_state_dict(torch.load('parameters/l-s-encoder.pt'))
		classifier.load_state_dict(torch.load('parameters/large-source-classifier-final.pt'))
	else:
		# encoder.load_state_dict(torch.load('parameters/s-l-target-encoder-' + str(epoch)+'.pt'))
